...
export async function generateBriefing(projectId: string): Promise<BriefStats> {
  ...
  await execute(
    ...
    [runId, projectId, process.env.DEPOT_BRIEFER_AGENT_ID ?? null, BRIEFER_AGENT.model],
  );
  ...
  try {
    ...
    if (entries.length === 0) {
      await execute(
        ...
        [JSON.stringify({ projectId, entryCount: 0, skipped: "no entries" }), runId],
      );
    ...
    await execute(
      `insert into briefing
			 ...
			 values ($1, $2, $3, $4, $5, $6)`,
      [briefingId, projectId, result.content, result.stateOfMind, runId, entries.length],
    );

    await execute(
      `update analysis_run set status = 'done', finished_at = now(), stats = $1 where id = $2`,
      [JSON.stringify({ projectId, entryCount: entries.length, briefingId }), runId],
    );
