...
export async function ingestUpload(
  ...
  for (const file of payload.files) {
    const outcome = await db.transaction((tx) => ingestFile(tx, payload, auth, file));
  ...
  for (const projectId of projectsTouched) {
    await db.update(project).set({ lastActivityAt: nowTs }).where(eq(project.id, projectId));
  }
...
async function ingestFile(
  ...
  const result = await ingestRecords(sess.id, file.providerSessionId, fileId, file.records, tx);
