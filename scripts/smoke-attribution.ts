...
function syntheticFile(sessionId: string) {
	...
	return {
		...
		records: [
			{ ...base, uuid: newId("evidence"), type: "user", parentUuid: null, message: { role: "user", content: "set up the db" } },
			{ ...base, uuid: newId("evidence"), type: "assistant", message: { role: "assistant", model: "claude", content: [{ type: "text", text: "done" }] } },
		],
	};
}
...
async function main() {
	...
	const { token: tokA } = await issueIngestToken({ label: "A", personId: personA, organizationId: orgA });
	const { token: tokB } = await issueIngestToken({ label: "B", personId: personB, organizationId: orgB });
	...
	const projects = await db
		.select({ id: schema.project.id, org: schema.project.organizationId, slug: schema.project.slug, remote: schema.project.canonicalRemote })
		.from(schema.project)
	...
	const sessions = await db
		.select({ org: schema.session.organizationId, person: schema.session.personId, project: schema.session.projectId })
		.from(schema.session);
	...
	console.log(`SESSIONS STAMPED w/ org+person: ${sessionsStamped ? "PASS ✅" : "FAIL ❌"}`);
