...
```bash
claude --plugin-dir /path/to/depot.ingram.tech/plugin
```

**Via a marketplace** (this directory is also a single-plugin marketplace):

```text
/plugin marketplace add /path/to/depot.ingram.tech/plugin
/plugin install depot@depot
```

or add the marketplace by git repo, then install `depot`.
